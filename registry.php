...
<body class="bg-gray-100 min-h-screen flex">
    ...
    <!-- Sidebar -->
<aside class="bg-blue-600 text-white w-64 space-y-6 py-7 px-2 fixed inset-y-0 left-0 top-0">
    ...
    <!-- Main Content -->
       <div class="flex-1 flex flex-col ml-64">
        ...
        <button onclick="document.getElementById('formModal').classList.remove('hidden')" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-blue-700">
            Add Student
        </button>
        </div>

         <div class="mb-4 text-center">
            <label for="courseSelector" class="block font-bold mb-2">Select Course:</label>
            <select id="courseSelector" class="border p-2 rounded mx-auto block" onchange="filterTableByCourse()">
...
<div id="studentsTable" class="overflow-x-auto bg-white rounded shadow-md p-4 hidden">
    <table class="w-full border-collapse border border-gray-200">
        ...
        <tbody>
            <?php foreach ($students as $student): ?>
                <tr data-course="<?php echo htmlspecialchars($student['course']); ?>">
...
<script>
    function filterTableByCourse() {
        ...
        if (selectedCourse) {
            ...
            rows.forEach(row => {
                if (row.getAttribute('data-course') === selectedCourse) {
    ...
    <script>
        ...
        new Chart(ctx1, {
            type: 'bar',
            data: {
                labels: coursesData.map(course => course.course),
                datasets: [{
                    label: 'Number of Students',
                    data: coursesData.map(course => course.count),
        ...
        new Chart(ctx2, {
            type: 'pie',
            data: {
                labels: genderData.map(gender => gender.gender),
                datasets: [{
                    data: genderData.map(gender => gender.count),
                    backgroundColor: ['#4A90E2', '#E94A87'],
                }],
            },
        });
    </script>
